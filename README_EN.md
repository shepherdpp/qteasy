...
qt.run(op)
```
The live trading program of `qteasy` is actually a timing task trigger, which can trigger the running of the trading strategy regularly, submit the strategy to the simulated trading Broker, read the real-time price, simulate the transaction, and automatically record the transaction process.
