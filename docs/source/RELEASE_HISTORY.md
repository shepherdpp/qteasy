...
- Added automatic rotation for trade_log and trade_summary files: before writing new logs, older `trade_log_*.csv` and `trade_summary_*.csv` files are removed according to the configured retention period to avoid unbounded growth of the log directory. This only affects on-disk files; no new config keys or API changes.
