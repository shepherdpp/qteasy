...
.. list-table:: qteasy Configuration Variables
   ...
   * - ``benchmark_asset_type``
     - 1
     - ``IDX``
     - | [DEPRECATED] 基准收益的资产类型。
       | 从2.0起，基准资产类型会根据 ``benchmark_asset`` 自动推断，建议不再显式配置该参数。
   * - ``benchmark_dtype``
     - 1
     - ``close``
     - | [DEPRECATED] 作为基准收益的资产的价格类型。
       | 从2.0起，基准价格类型会根据回测/优化时间表及资产类型自动推断，建议不再显式配置该参数。
   * - ``report``
     - 1
     - ``True``
     - | 为True时打印运行结果报告
   ...
   * - ``maximize_cash_usage``
     - 4
     - ``True``
     - | [DEPRECATED] 回测交易时是否最大化利用同一批次交易获得的现金。
       | 该参数在当前版本中已经不再生效，将在未来版本中移除，建议不要再依赖此配置。
   * - ``PT_signal_timing``
     - 3
     - ``lazy``
