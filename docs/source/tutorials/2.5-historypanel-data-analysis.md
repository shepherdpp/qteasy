...
> **定位**：`HistoryPanel` 是**轻量因子研究与粗验**容器，便于「玩数据」与目视筛因子；**完整回测**（费用、交割、MOQ、信号类型）仍须交给 `Operator` + Backtester，见 **§9**。

---

## 0. 30 分钟最小闭环（S1.2）

若你刚完成 [教程 2.0：获取并管理金融数据](2.0-get-data.md) 中的**最小数据集下载**，希望先跑通「取数 → 出图 → 简单因子 → 与基准粗对比」再深入阅读下文，可按本节 checklist 操作。

**前置**：本地已配置 `tushare_token`，且已下载 [2.0 最小数据集清单](2.0-get-data.md#最小数据集清单供-25-使用) 中的表（至少含 `index_daily` 或 `stock_daily`）。

**本节目标**：

- [ ] 用 `get_kline` 拉取多标的 + `000300.SH` 基准，得到 `HistoryPanel`
- [ ] `research_preset` 拼列并 `plot` 出图
- [ ] `returns` + 截面 `zscore(method='cs')`
- [ ] `portfolio(..., benchmark=...)` 做**研究向**组合对照（非账户回测）
- [ ] 理解边界：`portfolio` / `cum_return` **不含**交易成本、交割、MOQ

**一键脚本**（与下面代码等价，含分步 `print`）：

```bash
python examples/data_playground_e2e.py
```

**Notebook 最小代码**（与脚本逻辑一致）：
