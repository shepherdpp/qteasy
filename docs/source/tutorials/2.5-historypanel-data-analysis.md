...
若 `get_kline` 返回时间长度很短或报错，请先回到 [2.0](2.0-get-data.md) 补齐数据。**§1 及以下各节**在 §0 跑通后可按需跳读；需要系统 API 说明时再展开。

---

## 1. 准备数据：获取一个 HistoryPanel

> 若 **§0 已跑通**，本节示例可跳过，直接进入 **§2** 或你关心的专题节。

假设我们已经按 [2.0 最小数据集](2.0-get-data.md#最小数据集清单供-25-使用) 下载了指数或个股日线，下面以沪深 300（`000300.SH`）近一年为例，通过 `qt.get_kline()` 或 `qt.get_history_data()` 获取 `HistoryPanel`：
