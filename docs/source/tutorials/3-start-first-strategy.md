...
### 交易日志/交易报告详解
只要我们设置环境变量`trade_log=True`，系统就会在每次回测后生成两份文件：交易日志文件和交易明细报告。这两份报告的保存路径由配置项`trade_log_file_path`控制，默认在`QT_ROOT_PATH/tradelog/`路径下。该路径支持相对路径、绝对路径以及以`~`开头的家目录路径，且可通过`qt.configure(trade_log_file_path='...')`在运行时修改并立即生效（热修改），无需重新导入。例如：
