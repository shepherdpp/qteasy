...
在[上一篇教程](2.0-get-data.md)中，我介绍了如何配置本地数据源，查找、下载金融数据到本地，并从本地数据源中提取数据。如果还没有完成这一步的朋友，请移步前一篇教程了解如何下载和操作数据。
...
从回测的结果可以很容易看出，这个策略是跑赢了沪深300大盘指数的，在这十年间沪深300的年化收益率只有可怜的5%左右（4.84%），甚至比某些收益较高的定期产品都不如，而我们这个策略的投资年化收益率达到了19.86%，十年间总资产从十万元达到了六十多万元，翻了六倍多
...
### 交易日志/交易报告详解
只要我们设置环境变量`trade_log=True`，系统就会在每次回测后生成多份 CSV（至少包括交易日志与交易明细报告；当前版本还会在启用该选项时写入净值曲线 `value_curve_*` 等，以实际生成为准）。这些文件的保存路径由配置项`trade_log_file_path`控制，默认在`QT_ROOT_PATH/tradelog/`路径下。该路径支持相对路径、绝对路径以及以`~`开头的家目录路径，且可通过`qt.configure(trade_log_file_path='...')`在运行时修改并立即生效（热修改），无需重新导入。例如：

- Windows 指定到其他盘目录：`qt.configure(trade_log_file_path='C:\\qt_trade_logs\\')`
- macOS/Linux 指定到家目录：`qt.configure(trade_log_file_path='~/qt_trade_logs/')`

**磁盘保留策略**：配置项 **`trade_log_keep_days`** 默认为 **3**，表示在 **每次新进程导入 `qteasy` 之后** 会按该天数清理 `trade_log_file_path` 目录下过期的 `trade_log_*`、`trade_summary_*` 与 `value_curve_*` 等回测 CSV（不在每次回测写文件前自动清理）。若需长期保留全部历史文件，请将 **`trade_log_keep_days`** 设为 **`None`** 或 **小于等于 0**；也可随时调用 **`qt.rotate_trade_logs(days=...)`** 手动清理。

> 如果要查看当前的日志文件保存路径，请使用：
> ```python
> import qteasy as qt
...
打开路径文件夹，可以看到历次回测的报告，所有报告都保存为csv文件，便于用Excel打开，常见类型包括：

- **交易日志**：以“trade_log”打头是交易日志文件，罗列了每一次策略运行的详细信息
- **交易报告**：以“trade_summary”打头的是交易报告，里面罗列了每一笔交易的信息
- **净值曲线**（若已生成）：以“value_curve”打头，记录完整净值序列等
...
- **0, trade signal 交易信号**: 记录这次运行后产生的交易信号（交易信号是一组数字，经信号解析后变成交易订单，详见[交易信号](../manage_strategies/3.%20trade%20signals.md)）
...
- **表头区域**：表头区域第一列记录每一次运行的日期和时间，第二列记录每一次运行的策略组名称（详情参见[策略组](../manage_strategies/4.%20group_and_blender.md)），第三列标注每一行的名称，
...
- **持股记录**：这个区域里显示投资股票池中每一个股票的持股数量、交易数量、交易价格等等信息，每一列显示一支股票的数据，列数与股票池中的股票数量相等

基于上面的理解，我们可以分析一下策略的运行过程，看看策略是如何进行选股和换股的

![在这里插入图片描述](img/trade_log_explain.png)

如上图，我们可以以前四天的运行记录深入理解策略的运行方式：

- 第一天：初始资金投入10万元，投资开始启动，同时我们可以看到当天的沪深300和创业板指的涨幅分别为0.76%和1.83%，根据策略规则，我们在收盘时买入86.56股创业板指，消耗资金99996.32元，交易后持仓86.56股创业板指；
- 第二天：创业板指和沪深300指数涨幅分别为-0.77%和-0.56%，两相比较仍然是创业板指胜出，因此持仓不变；
- 第三天：沪深300指数涨幅反超创业板指，此时交易信号出现反转，同时产生了沪深300的买入信号和创业板的卖出信号，不过此时只有创业板指卖出信号得到执行，全仓卖出86.56股，但沪深300的买入信号无法执行，因为此时持有现金余额不足，只有3.68元可用。
- 第四天：交易信号持续反转，此时沪深300买入信号发挥作用，全仓买入31.01股沪深300指数
- ...

按照上面提到的方式，交易日志文件完整地记录了十年里一共2431个交易日的全部交易信息，从而使我们可以仔细分析每一笔交易的得失。
...
![在这里插入图片描述](img/trade_log_disaster.png)

所以其实这时候我们的策略仍然选择了正确的指数，只不过因为两个指数都在跌，我们的策略选择了跌的少的那一个持有，减少了我们的损失。

但是正是这一段时间的操作，造成了我们十年投资历史中最大的回撤事件：从2025年6月一直持续到2020年，深度套牢长达五年！！

#### 交易报告解读

交易日志虽然事无巨细地包含了所有的信息，但有时候我们并不需要察看那么多的数据，这时就可以使用交易报告了。

交易报告是交易日志文件的紧凑版本，它忽略了所有没有实质性交易的策略运行记录，只保留了所有的实质性交易记录，包括交易日期、买卖方向、交易份额、价格、总金额、交易费率等等信息，由于只记录有交易的实际发生，因此信息更加紧凑：

![在这里插入图片描述](img/trade_summary_overview.png)

从上图可以看到，交易报告的每一行记录一条实质交易记录，如果同一天发生多条交易记录，那么他们会被记录在不同的行。

交易报告的每一行里都记录了交易信号、交易价格、交易数量、现金变动、交易费率、交易后持仓数量等等信息，列名称包含的信息与交易日志完全相同；

## 策略的改进思路

综合上面我们对交易策略的分析，我们发现，这个交易策略虽然达到了较好的投资效果，但是在控制回撤方面，由于没有避开股灾区间，导致深度套牢。
