...
**本篇在数据体验路径中的位置**：[1 安装与配置](1-get-started.md) → **2.0 获取并管理金融数据（本篇）** → [2.5 玩数据与因子分析](2.5-historypanel-data-analysis.md) →（可选 [2.6](2.6-historypanel-advanced-vertical-timeaxis.md) / [2.7](2.7-historypanel-advanced-horizontal-multifactor.md) / [2.8](2.8-historypanel-advanced-event-kline-pattern.md)）→ [3 第一个策略回测](3-start-first-strategy.md)。
...
## 从 DataFrame 到 HistoryPanel（桥接 2.5）
...
更系统的「玩数据」与因子分析见 [教程 2.5 §0](2.5-historypanel-data-analysis.md#0-30-分钟最小闭环s12)；可运行示例见 `examples/historypanel_basic_plot.py` 与 `examples/data_playground_e2e.py`。
...
若计划接着学习 [教程 2.5](2.5-historypanel-data-analysis.md)，建议在本篇至少下载下列表与区间（与 [快速上手指南](../getting_started.md) 及 `examples/data_playground_e2e.py` 对齐）：
...
下载完成后，可运行 `python examples/data_playground_e2e.py` 验证 [2.5 §0](2.5-historypanel-data-analysis.md#0-30-分钟最小闭环s12) 最小闭环。
...
- [教程 2.5：使用 HistoryPanel 操作和分析历史数据](2.5-historypanel-data-analysis.md) — 从 **§0 三十分钟闭环** 开始，完成「玩数据 → 简单因子」

**若已熟悉数据研究、希望直接进入回测**：

- [教程 3：创建并回测第一个策略](3-start-first-strategy.md)
