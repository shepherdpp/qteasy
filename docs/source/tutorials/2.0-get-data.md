...
**本篇在数据体验路径中的位置**：[安装与配置](1-get-started.md) → **获取并管理金融数据（本篇）** → [HistoryPanel 数据面板基础操作](2.4-historypanel-basics.md) → [玩数据与因子分析](2.5-historypanel-data-analysis.md) →（可选 [纵向择时](2.6-historypanel-advanced-vertical-timeaxis.md) / [横截面选股](2.7-historypanel-advanced-horizontal-multifactor.md) / [事件型因子](2.8-historypanel-advanced-event-kline-pattern.md)）→ [第一个策略回测](3-start-first-strategy.md)。
...
- **数据通道（二选一或组合）**：
  - 使用 **Tushare**：注册`tushare pro`账户并确保有一定的积分（大多数高级数据需要较多积分才能下载），在`qteasy.cfg`中配置`tushare_token`
  - 使用 **AKShare 或其他通道**：安装 `pip install akshare`（若选 AKShare），无需 Tushare token；部分表覆盖范围小于 Tushare，详见[数据通道文档](../manage_data/10.%20data_channels.md)
...
- `stock_basic` - 股票基础信息（文档中有时统称「basics」），包括沪深股市所有股票的基本信息
- `index_basic` - 指数基础信息，包括沪深股市所有指数的基本信息

> `refill_data_source(tables=...)` 使用**数据表名**（如 `stock_basic`），与上列一致。
...
我们可以调用`refill_data_source`函数下载交易日历和基础数据。这个函数是`qteasy`的标准数据下载接口函数，所有的历史数据类型均可以通过此接口下载。这个函数的基本参数是`tables`，传入数据表的名称即可下载相应的数据到本地存储了。使用`refill_data_source`下载交易数据时，`qteasy`会自动进行数据清洗，排除重复数据，去除错误数据，发生错误自动重试，并将下载的数据合并到本地数据表中。

`qteasy`支持通过 **`tushare`、`akshare`、`eastmoney`、`sina`** 等多个内置通道下载数据（参数 `channel`，未指定时默认为 `tushare`）。不同通道支持的表范围与前置条件不同，切换方式与能力对照见[数据通道文档](../manage_data/10.%20data_channels.md)。
...
我们可以使用`qt.get_history_data()`函数来获取股票的量价等 **History**（时间 × 标的）数据。这是 History 形状的通用入口；宏观/基准请用 `get_reference_data`，行业等截面属性请用 `get_static_data`（见下文「按形状取数」）。在函数的参数中指定数据的类型（通过数据类型 ID）、股票的代码以及其他参数，就可以获取相应的数据了。如果要获取刚刚下载的K线价格，需要设置数据类型为`"open, high, low, close, vol"`以获取开盘价、最高价、最低价、收盘价和交易量：
...
## 从 DataFrame 到 HistoryPanel（桥接后续教程）

上文 `get_history_data()` 默认返回按标的分组的 `DataFrame` 字典；做**多标的因子研究**时，更推荐直接拿到三维 `HistoryPanel`（标的 × 时间 × 指标）：

```python
import qteasy as qt

# 方式一：get_kline 一行取 OHLCV 面板（推荐入门）
hp = qt.get_kline(
    shares='000300.SH',
    start='20230101',
    end='20231231',
...
# 方式二：get_history_data 显式指定 as_data_frame=False
...
更系统的容器操作见 [HistoryPanel 数据面板基础操作](2.4-historypanel-basics.md)；「玩数据」与因子分析见 [使用 HistoryPanel 操作和分析历史数据](2.5-historypanel-data-analysis.md)；可运行示例见 `examples/historypanel_basic_plot.py` 与 `examples/data_playground_e2e.py`。

## 数据类型`DataType`的查找与按形状取数

前面提到过，`qteasy`把可直接使用的信息封装为稳定的 **DataType 信息 ID**（用户日常写字符串；不必每次构造对象）。每种类型由名称、频率与资产类型唯一确定。

概念说明、精选常用类型与完整内置清单，见：

- [DataType 概念章（manage_data）](../manage_data/02.%20datatypes.md)
- [内置 DataType 完整清单](../references/datatypes/index.md)
- [DataType API](../api/data_types.rst)

### 用 `find_history_data` 查找

`qt.find_history_data()` 可按英文名、中文描述或通配符搜索。结果会标出 `kind` 与推荐入口（`get_history_data` / `get_reference_data` / `get_static_data` / `none`）——**不要**假定每一行都能用 `get_history_data` 加载。
...
use recommended_api for each row (get_history_data / get_reference_data / get_static_data / none):
------------------------------------------------------------------------
          name freq asset        table column         desc     kind               usable_in               api
data_id                                                                                                       
close    close    d     E  stock_daily  close  股票日K线 - 收盘价  history  history_panel,strategy  get_history_data
close    close    d   IDX  index_daily  close  指数日K线 - 收盘价  history  history_panel,strategy  get_history_data
  ...
========================================================================
```

宏观与截面属性同样能搜到，但推荐入口不同：

```python
>>> qt.find_history_data('cn_gdp')   # 推荐 get_reference_data
>>> qt.find_history_data('industry') # 推荐 get_static_data
```

查到 ID 后：看所属数据表，用 `refill_data_source(tables=...)` 下载，再按推荐入口取数。

### 按形状取数（短例）

```python
# History：时间 × 标的（可进 HistoryPanel）
hist = qt.get_history_data(
    htype_names='close, pe',
    shares='000001.SZ, 000002.SZ',
    start='20230101', end='20230131',
)

# Reference：仅时间（宏观 / 把某标的当基准）；不传股票池
ref = qt.get_reference_data('cn_gdp', start='20100101', end='20231231')
# 基准造法：qt.get_reference_data('close-000300.SH', freq='d', asset_type='IDX', ...)

# Static：仅标的（行业、上市日等截面）
static = qt.get_static_data('industry, list_date', shares='000001.SZ, 600519.SH', asset_type='E')
```

清单里若 `usable_in=none`（例如部分文本型 History），今天没有一等入口，研究时请直接读底层表，不要强行编入 HistoryPanel。

### 简单介绍`DataType`对象（专家层）

日常取数与策略声明优先使用**字符串 ID**。需要精细控制时，也可以构造 `DataType` 对象；完整说明见 API 文档。策略里声明 History/Reference 字符串后，在 `realize()` 中用 `get_data` 取窗口即可——**不要**把 Static（如 `industry`）写进策略 `data_types`。

## 最小数据集清单（供后续 HistoryPanel 教程使用）

若计划接着学习 [HistoryPanel 基础操作](2.4-historypanel-basics.md) 与 [玩数据与因子分析](2.5-historypanel-data-analysis.md)，建议在本篇至少下载下列表与区间（与 [快速上手指南](../getting_started.md) 及 `examples/data_playground_e2e.py` 对齐）：

| 数据表                          | 用途                  |
|------------------------------|---------------------|
| `trade_calendar`             | 交易日历                |
| `stock_basic`, `index_basic` | 证券检索                |
| `index_daily`                | 基准指数 `000300.SH` 日线 |
| `stock_daily`                | 示例个股池日线（见下）         |

**推荐一次性命令**（Tushare 通道；日期可按需调整）：
...
下载完成后，可先阅读 [HistoryPanel 基础操作](2.4-historypanel-basics.md)，再运行 `python examples/data_playground_e2e.py` 验证 [玩数据最小闭环](2.5-historypanel-data-analysis.md#数据检查快速跑通最小闭环)。
...
至此，我们已经初步了解了`qteasy`中对数据的管理方式，了解了数据下载的方法，学会了提取数据、显示 K 线图，以及通过 `HistoryPanel` 桥接到多标的研究。

**建议下一篇**（数据体验主路径）：

- [HistoryPanel 数据面板基础操作](2.4-historypanel-basics.md) — 结构、切片、扩列、对齐与 DataFrame 互转
- [使用 HistoryPanel 操作和分析历史数据](2.5-historypanel-data-analysis.md) — 从「数据检查」闭环开始，完成「玩数据 → 简单因子」

**若已熟悉数据研究、希望直接进入回测**：

- [创建并回测第一个策略](3-start-first-strategy.md)

关于`qteasy`的更多介绍，请参见 [qteasy 文档](https://qteasy.readthedocs.io)。