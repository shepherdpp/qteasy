...
`qteasy`是一个完全本地化部署和运行的量化交易分析工具包，[Github地址在这里](https://github.com/shepherdpp/qteasy)，并且可以通过pip安装：
...
- 金融数据的**获取、清洗、存储**以及**处理、可视化、使用**
- 量化交易策略的创建，并提供大量**内置基本交易策略**
- 向量化的**高速交易策略回测**及交易结果评价，**避免未来函数**的内核设计
- 交易策略参数的**优化、评价、部署、以及实盘运行**
...
- **策略需要的数据** ——策略需要的数据输入；例如，需要过去10天的日K线数据，还是过去一年的市盈率？在qteasy中，策略所需的数据量可以以`Strategy.strategy_data_types` 等属性完全自由定义，并指定数据类型、频率以及窗口长度
...
![在这里插入图片描述](img/MAV%20Cross%20Strategy.png)
...
![在这里插入图片描述](img/MAV%20Cross%20Strategy2.png)
...
![在这里插入图片描述](img/Data%20window%20explained.png)
...
**未完待续**
## 用`FactorSorter`定义Alpha选股策略的第二种实现方法
