...
> **入口怎么用（请先读这一段）**：路径 2 的 `get_kline` / `get_history_data` 是当前稳定的公开入口，示例可直接跟跑（需已按 [获取数据](../tutorials/2.0-get-data.md) 准备本地数据）。宏观、基准与证券属性**已登记在清单里**，但消费形状与 History 不同，**不要**假定它们都能用 `get_history_data` 编进 `HistoryPanel`。路径 3、4 的入口名 `get_reference_data` / `get_static_data` 已作为用户契约锁定，下文给出调用骨架；可运行示例将随公开函数一并补齐。
...
# 示意骨架：宏观序列不需要 shares；基准写在 ID 里
# （参数名与 get_history_data 对齐，以公开 API 为准）
# gdp = qt.get_reference_data('cn_gdp', start='20100101', end='20231231')
# bench = qt.get_reference_data('close-000300.SH', start='20230101', end='20230131')
```
...
- **可用性**：完整清单会保留全部内置类型，其中一部分今天没有一等用法（目录里看得到，但没有推荐入口）。检索结果请按描述与入口提示选用，不要一律 `get_history_data`。
