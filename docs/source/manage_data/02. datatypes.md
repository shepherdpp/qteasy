...
> **入口怎么用（请先读这一段）**：路径 2 的 `get_kline` / `get_history_data`，以及路径 3 的 `get_reference_data`，是当前稳定的公开入口（需已按 [获取数据](../tutorials/2.0-get-data.md) 准备本地数据）。宏观与基准走 Reference，**不要**用 `get_history_data`「顺便」取。路径 4 的 `get_static_data` 名称已锁定，公开函数随后续 Phase 落地。
