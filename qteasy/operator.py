...
class Operator:
    ...
    def __init__(self, selecting_types=None,
        ...
        目前在Operator对象中可以支持的策略有三种类型，而每种类型的策略都有不同的种类，在同一个Operator对象中支持无数个同一类型策略，
        通过策略混合的方式将结果汇总到一起。三种类型的策略分别如下：

            1,  单品种多空位置策略，目前称为择时策略

                这类策略的共同特征是对投资组合中的所有投资产品逐个考察其历史数据，根据其历史数据，在历史数据的粒度上生成整个时间段上的
                多空位置时间序列。用>0的数字表示多头头寸，<0的数字代表空头头寸，0代表中性头寸。同时，在生成不同投资产品的多空位置时间
                序列时，可以应用相同的择时参数，也可以应用不同的择时参数。
                为了避免前视偏差，每个择时多空策略仅利用一小段有限的历史数据（被称为时间窗口）来生成每一个时间点上的多空位置，同时确保
                时间窗口总是处于需要计算多空位置那一点的过去。这种技术我称为"时间窗口滚动"。当然，时间窗口滚动的计算方式注定了计算量巨
                大，因此未来对此策略的改进主要在效率方面。

                这种策略生成的结果是多空择时蒙板

            2,  周期运行比例分配策略，目前称为选股策略

                这类策略的共同特征是周期性运行，且运行的周期与其历史数据的粒度不同。在每次运行时，根据其历史数据，为潜在投资组合中的每
                一个投资产品分配一个权重，并最终确保所有的权重值归一化。权重为0时表示该投资产品被从组合中剔除，而权重的大小则代表投资
                过程中分配投资资金的比例。
                目前选股策略的运行仍然依赖低效的for-looop进行。在运行频率不高时对效率影响不大，但随着运行频率升高，效率问题会变得突出。

                这种策略生成的结果是选股蒙板

            3,  单品种信号生成策略，目前称为风控策略

                这类策略的共同特征是直接根据历史数据生成交易信号。按照以前的设想，这类策略被用于风控。但是根据最新的知识，风控策略并不
                需要做成单独的一个类型，相反，风控只是策略的一个目标，而不是一个类型，不管是通过单品种多空位置策略、周期运行比例分配策
                略、还是但品种信号生成策略，都可以建立一个以风控为目的的策略。真正让这类策略与其他两类策略有所区别的，在于这类策略直接
                生成交易信号，而不是多空位置或资金分配比例，前者可以直接进入回测模块进行交易模拟，而后者必须根据当前持仓等信息，判断并
                计算出交易信号，进而才能被用于交易或交易模拟。

                这种策略生成的结果是交易信号矩阵

                正如风控是策略的目标而不是类型一样，择时或选股同样也是策略的目标，单纯的择时或选股都可以通过第一类或第二类策略来实现。

                由于以前的设想不尽合理，因此目前本策略还被称为风控策略，而其实现方式太过于简单化，使用全局历史数据向量化计算得到。这种
                方式无法避免前视偏差，而且无法针对不同的投资产品应用不同的策略参数，应作重大修改。同时所有策略类型的名称都应该逐步与
                目的脱钩，而与它的运作方式挂钩。

                这类策略应该被改造为与但品种多空位置策略类似，甚至操作方式完全一样，唯一的区别是输出值>0时代表开多仓或平空仓，输出值<0
                时代表开空仓或平多仓，而买入、卖出类型和数量则需要额外表示
                （现在还没有想清楚，如果输出形式与单品种多空策略相同的话，也许这种策略本身就可以合并到单品种多空策略中去）



        ==策略的混合==

        在Operator对象中，三种类型的策略都可以有无限多个，分别计算其策略结果后通过某种方式混合成为最终的蒙板。

        三种策略生成的蒙板或矩阵有各自不同的混合方法：

            1,  多空蒙板的混合方式：
                多空蒙板的混合方式由多空混合字符串来确定，字符串的格式为"[chg|pos]-0/9|cumulate"(此处应该使用正则表达式)
        ...
        的。
        在一个Operator对象中，作为容器可以容纳任意多个任意类型的策略，但是相同类型的策略必须编为一组，这一组内的所有策略可以引用不同的
        历史数据，但是生成同样大小尺度的结果（也就是说，生成的结果有相同的历史区间，相同的时间粒度），最后这些结果被通过某种方法"混合"
        起来，形成每个策略类型的最终的结果，第一种策略生成的结果叫做多空模版，第二种策略生成的结果叫选股蒙板，第三种策略生成的是交易信号。
        不同类型的结果又再次被组合起来，变成整个Operator对象的输出。

        目前采用的组合方式是：

        mask_to_signal(多空模版 * 选股蒙板) + 交易信号（目前叫风控信号）
        其中mask_to_signal()函数的作用是将蒙板转化为交易信号，这样输出的就是交易信号

        未来将第三类策略升级为但品种信号生成策略后，信号的组合方式就应该变为：
